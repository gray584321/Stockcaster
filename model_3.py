...
# New imports for hyperparameter optimization
import sys            # Used to check for command-line flag
...
def objective(trial):
    """
    Objective function wrapped for Optuna hyperparameter search.
    We suggest different hyperparameters and train for a few epochs,
    then return the average validation loss.
    """
    ...
    use_prob_sparse = trial.suggest_categorical("use_prob_sparse", [True, False])
    
    # Use fixed model/data parameters from the __main__ section
    input_dim = 12
    encoder_length = 96
    decoder_length = 48
    ...
    # Set up optimizer with the suggested learning rate
    optimizer = Adam(model.parameters(), lr=learning_rate)
    
    # Train with a reduced number of epochs for fast hyperparameter evaluation
    num_epochs_trial = 5
    train_model(model, train_loader, val_loader, num_epochs_trial, optimizer, criterion, device)
...
if __name__ == "__main__":
    ...
    prediction_length = 24
    batch_size = 128
    num_epochs = 50  # Set full training epochs for final training
    default_learning_rate = 4e-5  # Default if not using hyperparameter tuning
    ...
    # Initialize criterion; if using Optuna, it will be used inside objective().
    criterion = nn.MSELoss()
    
    # Check if the script is invoked with the "optuna" flag
    if "optuna" in sys.argv:
        ...
        # Rebuild the model with the best hyperparameters and train for full epochs
        ...
        model = Informer(
            ...
            decoder_length=decoder_length,
            prediction_length=prediction_length,
            use_prob_sparse=best_params["use_prob_sparse"]
        )
        model.to(device)
        ...
        # Default model training with fixed hyperparameters
        d_model = 64
        d_ff = 256
        nhead = 8
        enc_layers = 3
